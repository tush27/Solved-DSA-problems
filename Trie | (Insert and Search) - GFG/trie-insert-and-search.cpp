...
bool containkey(TrieNode*node,char ch){
    return (node->children[ch-'a']!=NULL);
}
void put(TrieNode*node,char ch,TrieNode*newnode){
    node->children[ch-'a']=newnode;
    newnode->isLeaf=false;
}
TrieNode*get(TrieNode*node,char ch){
...
void insert(struct TrieNode *root, string key)
{
    ...
    for(int i=0;i<key.size();i++){
        if(!containkey(node,key[i]))
        put(node,key[i],new TrieNode());
